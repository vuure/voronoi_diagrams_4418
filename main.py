...
#def calculateAndSort_range(points):

    #for indexNumber, point in enumerate(points, start=0):
        #xValue = point[1]
        #yValue = point[2]
        #distance = math.sqrt(xValue ** 2 + yValue ** 2)

        #points[indexNumber].append(round(distance,2))

    #points.sort(key=lambda x: x[3])

    #return points

#calculated_ranges = calculateAndSort_range(points_list)

#print(calculated_ranges[1][0])

#while 0 < len(calculated_ranges):
    #current_closest = calculated_ranges[1][0]
